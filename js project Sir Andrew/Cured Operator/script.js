...
const validRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;  //mail text required text/num/@ .text
...
function renderTable() {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    editBtn.className = "edit-btn";                                //edit btn
    const deleteBtn = document.createElement("button");           //create btn
    deleteBtn.className = "delete-btn";                           //delete btn
    idTd.innerText = user.id;                                      //id means s.no.1,2,3,4,5,6
...
function addUser() {
  ...
  if(email.match(validRegex)){
    if(name && email != null){
      ...
      alert("Name is Required");                //if left blank
    }
  }else{
    alert("Invalid email address!");             //if left blank,miss @
  }
}
